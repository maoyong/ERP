...
class Delivery extends Controller
{
	...
	protected function filter(&$map)
	{
	    ...
	    $map['_field'] = 'a.id,a.order_no,a.compact_no,a.put_type,a.position,a.put_time,s.client_name,a.status';
	    $map['_func'] = function ($model) {
	    	$model->alias('a')->join('client s', 'a.supplier=s.id', 'left');
	    };
	...
	public function add(){
		...
		$this->view->assign('purchases', DB::name('sell')->field('id,compact_no')->where(['status'=>1, 'is_put' => 0, 'isdelete' => 0])->select());
	...
	public function beforeEdit(){
		if ($this->request->isAjax()) {
			...
			if ($data['status'] == 1) {
				...
				$store->updateStore($data['id'], 1, 0);//添加
			}else{
				$store = model('store', 'logic');
				$store->updateStore($data['id'], 1, 1);
			}
			return true;
		}
	...
	public function excel()
    {
        ...
        $data = DB::name('delivery')
        			...
        			->select();
        \Excel::export($header, $data, "出库列表", '2007');
    }
