...
class Auth
{
    ...
    public function check($name, $uid, $type = 1, $mode = 'url', $relation = 'or')
    {
     ...
     public static function AccessCheck($name, $uid, $type = 1, $mode = 'url', $relation = 'or')
    {
        if ($uid == 1) {
            return true;
        }
        ...
        $authList = Session::get('Auth/AuthList/gid'.$gid);
        if (empty($authList)) {
            return false;
        }
        if (empty($authList)) {
     ...
     */
    public function saveAccessList($uid, $gid,$type = 1){
         $authList = $this->getAuthList($uid, $type);
         Session::set('Auth/AuthList/gid'.$gid, $authList);
         return true;
    }
}