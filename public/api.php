...
//												$parameter['collectionFormat'] = 'multi'; // url不带中括号
									...
									$responses = array();
									$pattern = '/responses\s*:\s*(?<name>\w+)\s*-\s*\{(?<type>\w+(?<array>\[\])?)\}\s*(=\s*((\[(?<enum>[^]]+)\])|(?<default>[^\s]+))\s*)?(?<summary>[^*]+)/i';
									if(preg_match_all($pattern, $func_contents, $matches)) {
										$names = $matches['name']; 		// 参数名称
										$types = $matches['type']; 		// 参数类型
										$summarys = $matches['summary']; // 参数说明
										$params_count = count($names);
										for($j = 0; $j < $params_count; $j++) {
											$response = ['description'=>trim($summarys[$j]), 'type' => $types[$j]];
											$responses[$names[$j]] = $response;
										}
									}
									$func['parameters'] = $parameters;
									if(!empty($responses)){
										$func['responses'][200]['description'] = '返回数据';
										$func['responses'][200]['schema']['items']['properties'] = $responses;
									}
									// 生成api访问路径
